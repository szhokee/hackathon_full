...
from ticket.views import CategoryAPIView, ProductModelViewSet
...
class CategoryTest(APITestCase):
    ...
    def setup_user(self):
        self.user = User.objects.create_user(email='test@test.com', password='test123', is_active=True)
...
class TicketTest(APITestCase):
    ...
    def test_post_product(self):
        image = open('media/products/maxresdefault.jpg', 'rb')
        data = {
        ...
        assert response.status_code == 201
        assert Ticket.objects.filter(title='test_product').exists()
